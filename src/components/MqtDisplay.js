...
const MqtDisplay = () => {
  ...
  useEffect(() => {
    ...
    allSounds.forEach((audio, index) => {
      audio.preload = 'auto';
      audio.volume = 1.0;
      audio.playbackRate = 1.0;

      // Add event listeners for better loading tracking
      audio.addEventListener('canplaythrough', () => {
        // Audio is fully loaded and can play through without buffering
        console.log(`Audio ${index + 1} ready for immediate playback`);
      });

      audio.addEventListener('error', () => {
        console.warn(`Audio file ${index + 1} failed to load`);
      });
      ...
      try {
        ...
        console.warn(`Failed to load audio file ${index + 1}:`, e);
      }
    });
  ...
  useEffect(() => {
    if (isRunning) {
      // Enhanced start sound playback
      const currentIndex = soundIndexes.current.start;
      const audio = startSounds.current[currentIndex];
      if (audio) {
        const playStartSound = async () => {
          try {
            ...
            soundIndexes.current.start = (currentIndex + 1) % 3;
          } catch (error) {
      ...
      setTime(prev => {
        ...
        if (countUp) {
          if (prev >= currentDuration) {
            // Timer finished - play end sound immediately with enhanced reliability
            const currentIndex = soundIndexes.current.end;
            const audio = endSounds.current[currentIndex];
            if (audio) {
              // Enhanced audio playback for minimal delay
              const playEndSound = async () => {
                try {
                  // Resume audio context if suspended (modern browsers)
                  ...
                  // Ensure audio is ready and reset position
                  audio.currentTime = 0;
                  ...
                  if (audio.readyState >= 2) { // HAVE_CURRENT_DATA or better
                    ...
                    // If not ready, wait for it to load then play
                    audio.addEventListener('canplay', async () => {
                      await audio.play();
                    }, { once: true });
                  }

                  soundIndexes.current.end = (currentIndex + 1) % 3;
                } catch (error) {
                  console.warn('End sound playback failed:', error);
                  // Fallback: try next audio instance
                  const fallbackIndex = (currentIndex + 1) % 3;
                  const fallbackAudio = endSounds.current[fallbackIndex];
                  if (fallbackAudio) {
          ...
          if (warnMode === '10s' && nextTime <= 10 && nextTime > 0) {
            // Enhanced warn sound playback
            const currentIndex = soundIndexes.current.warn;
            const audio = warnSounds.current[currentIndex];
            if (audio && audio.readyState >= 2) {
              audio.currentTime = 0;
              audio.play().catch(() => { });
              soundIndexes.current.warn = (currentIndex + 1) % 3;
            }
          }
  ...
  }, [isRunning, currentDuration, countUp, autoRestart, warnMode]);
    ...
    <div className={`mqt-display fullscreen theme-${theme}`} style={{ cursor: 'none' }} onClick={handleDisplayClick}>
      <svg className="circle-svg" viewBox="0 0 240 240">
        ...
        <circle
          ...
          cy="120"
          stroke="#666"
          strokeWidth={getResponsiveStrokeWidth(circleStyle === 'fat' ? 16 : circleStyle === 'bw' ? 2 : 8)}
          fill="none"
